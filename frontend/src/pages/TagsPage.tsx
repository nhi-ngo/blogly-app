...
const TagsPage: React.FC<TagsPageProps> = ({ isAuthenticated }) => {
  ...
  const handleAddTags = async () => {
    ...
    try {
      ...
      await tagService.createTags(newTags);
      await fetchTags();
      handleModalClose();
    } catch (err) {
            ...
            <TableBody isLoading={loading} loadingContent={<div>Loading tags...</div>}>
              {tags.map((tag) => (
                <TableRow key={tag.id}>
                  ...
                  <TableCell>
                    {isAuthenticated ? (
                      <Tooltip content={tag.postCount ? 'Cannot delete tag with existing posts' : 'Delete tag'}>
                        <Button
                          isIconOnly
                          variant="flat"
                          color="danger"
      ...
      <Modal isOpen={isOpen} onClose={handleModalClose}>
        <ModalContent>
          ...
          <ModalBody>
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {newTags.map((tag) => (
                  <Chip key={tag} onClose={() => handleRemoveNewTag(tag)} variant="flat" endContent={<X size={14} />}>
                    {tag}
                  </Chip>
                ))}
              </div>
          ...
          <ModalFooter>
            ...
            <Button color="primary" onClick={handleAddTags} isLoading={isSubmitting} isDisabled={newTags.length === 0}>
              Add Tags
            </Button>
          </ModalFooter>
        </ModalContent>
